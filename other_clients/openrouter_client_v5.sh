...
SERVER_URL="https://openrouter.ai/api/v1/chat/completions"  # Ministral-3-8B-Instruct-2512-Q4_K_M
SUMMARY_URL="https://openrouter.ai/api/v1/chat/completions" # Llama-3.2-1B-Instruct-Q4_K_M
...
while true; do
  ...
  printf "%b" "${GREEN}Enter message (2xEnter = send, Shift+Enter = newline):\n ${RESET}"
  ...
  RESPONSE=$(
    curl -N -s "$SERVER_URL" \
      ...
      grep --line-buffered "^{" | # Only lines starting with '{' (a valid json string) is passed to jq
